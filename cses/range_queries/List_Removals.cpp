...
struct SegmentTreeTemplate {
    ...
    inline StoreType sum(StoreType a, StoreType b) {
        return a + b;
    }
    ...
    int findFirstOneTree(int st, int end, int ind, int l, int r, int val) {
        if(st == end and l == r) return l;
        int mid;
        StoreType p1;
        mid = st + (end - st) / 2;
        if(r <= mid) return findFirstOneTree(st, mid, (ind << 1) + 1, l, r, val);
        if(l > mid) return findFirstOneTree(mid + 1, end, (ind << 1) + 2, l, r, val);
        p1 = queryTree(st, mid, (ind << 1) + 1, l, mid);
        if(p1 >= val) return findFirstOneTree(st, mid, (ind << 1) + 1, l, mid, val);
        return findFirstOneTree(mid + 1, end, (ind << 1) + 2, mid + 1, r, val - p1);
    }
    void build() {
        buildTree(0, n - 1, 0);
    }
    ...
    int findFirstOne(int l, int r, int val) {
        if(query(l, r) < val) return -1;
        return findFirstOneTree(0, n - 1, 0, l, r, val);
    }
};
...
int main(){
    ...
    while(m--) {
        lld x = sc();
        int ind = st.findFirstOne(0, n - 1, x);
        if(ind == -1) {
            assert(false);
        } else {
            printf("%d ", Val[ind]);
