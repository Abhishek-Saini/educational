...
// adding Arithmetic Progression to a range
...
struct SegmentTreeLazy {
    ...
    inline void apply(int ind, int st, int end, PA val) {
        Error(ind, st, end, val.first, val.second);
        if(ToPush[ind]) {
            Lazy[ind].first += val.first;
            Lazy[ind].second += val.second;
        }
        else {
            Lazy[ind] = val;
        }
        ToPush[ind] = true;
        lld cnt = end - st + 1;
        M[ind].totalSum += (2 * val.first + (cnt - 1) * val.second) * cnt / 2;
    }
    ...
    inline void push(int ind, int st, int end) {
        ...
        apply(b, mid + 1, end, {Lazy[ind].first + (mid - st + 1) * Lazy[ind].second, Lazy[ind].second});
    ...
    void updateTree(int st, int end, int ind, int updateLeftInd, int updateRightInd, PA val){
        ...
        updateTree(mid + 1, end, b, max(mid + 1, updateLeftInd), updateRightInd, PA(val.first + max(0, (mid - updateLeftInd + 1)) * val.second, val.second));
        // change here for how to merge
        M[ind] = combineNodes(M[a], M[b]);
    }
    ...
    void update(int updateLeftInd, int updateRightInd, PA val) {
        updateTree(0, n - 1, 0, updateLeftInd, updateRightInd, val);
    }
    StoreType query(int l, int r) {
        return queryTree(0, n - 1, 0, l, r).totalSum;
    }
};
