...
int main(){
    int n = scd(), q = scd();
    repI(i, 0, n - 1) A[i] = sc();
    SegmentTree neg(n, A), pos(n, A);
    repI(i, 0, n - 1) A[i] -= i;
    neg.build();
    repI(i, 0, n - 1) A[i] += 2 * i;
    pos.build();
    while(q--) {
        int type = scd();
        if(type == 1) {
            int k = scd();
            lld x = scd();
            pos.update(k - 1, x + k - 1);
            neg.update(k - 1, x - k + 1);
            A[k - 1] = x;
        }
        else {
            int k = scd() - 1;
            lld ans = neg.query(0, k) + k;
            if(k + 1 < n) ans = min(ans, pos.query(k + 1, n - 1) - k);
            prL(ans);
        }
    }
    return 0;
